...
class Temporal_Bottleneck_Layer(nn.Layer):
    def __init__(self, channel, temporal_window_size, bias, act, reduct_ratio, stride=1, residual=True, **kwargs):
        ...
        # ===
         #   nn.BatchNorm2D(inner_channel),不加momentum，下同
...
class SpatialGraphConv(nn.Layer):
    def __init__(self, in_channel, out_channel, max_graph_distance, bias, edge, A, **kwargs):
        ...
        #表述不一致
        '''
        self.A = self.A[:self.s_kernel_size]
        self.A = self.create_parameter(
            shape=self.A.shape, dtype='float32', attr=paddle.ParamAttr(initializer=nn.initializer.Assign(self.A), trainable=False)
        )
        ...
        if edge:
            ...
            self.add_parameter('edge', self.edge)
        else:
            self.edge = self.create_parameter(
...
class AconC(nn.Layer):
    ...
    def __init__(self, channel):
        ...
        # === 表述不一致
        #self.p1=paddle.randn([1, channel, 1, 1], dtype='float32')
        #self.p2=paddle.randn([1, channel, 1, 1], dtype='float32')
        #self.create_parameter(shape=self.p1.shape, dtype=str(self.p1.numpy().dtype), attr=paddle.ParamAttr(initializer=nn.initializer.Assign(self.p1), trainable=True))
        #self.add_parameter('p1', self.p1)
        #self.p2 = self.create_parameter(shape=self.p2.shape, dtype=str(self.p2.numpy().dtype), attr=paddle.ParamAttr(initializer=nn.initializer.Assign(self.p2), trainable=True))
        #self.add_parameter('p2', self.p2)
        #self.beta=paddle.ones([1, channel, 1, 1], dtype="float32")
        #self.beta=self.create_parameter(shape=beta.shape, dtype=str(beta.numpy().dtype), attr=paddle.ParamAttr(initializer=nn.initializer.Assign(beta), trainable=True))
